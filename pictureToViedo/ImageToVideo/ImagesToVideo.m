...
{
    [ImagesToVideo videoFromImages:images
                              toPath:path
...
+ (void)videoFromImages:(NSArray *)images
     ...
     animateTransitions:(BOOL)animate
      withCallbackBlock:(SuccessBlock)callbackBlock
{
    [ImagesToVideo writeImageAsMovie:images
                                toPath:path
                                  size:size
                                   fps:fps
                    animateTransitions:animate
...
+ (void)saveVideoToPhotosWithImages:(NSArray *)images
                  withCallbackBlock:(SuccessBlock)callbackBlock
{
    [ImagesToVideo saveVideoToPhotosWithImages:images
                                        withSize:DefaultFrameSize
                              animateTransitions:DefaultTransitionShouldAnimate
                               withCallbackBlock:callbackBlock];
}
    ...
    while (1)
    {
        ...
        if(writerInput.readyForMoreMediaData){
            ...
            if (i >= [array count]) {
                ...
                buffer = [ImagesToVideo pixelBufferFromCGImage:[array[i] CGImage] size:size];
            }
            
            if (buffer) {
                //append buffer
                
                BOOL appendSuccess = [ImagesToVideo appendToAdapter:adaptor
                                                          pixelBuffer:buffer
                                                               atTime:presentTime
                ...
                if (shouldAnimateTransitions && i + 1 < array.count) {
                    ...
                    for (double j = 1; j < framesToFadeCount; j++) {
                        ...
                        buffer = [ImagesToVideo crossFadeImage:[array[i] CGImage]
...
+ (CVPixelBufferRef)pixelBufferFromCGImage:(CGImageRef)image
    ...
    CGColorSpaceRef rgbColorSpace = CGColorSpaceCreateDeviceRGB();
    CGContextRef context = CGBitmapContextCreate(pxdata, imageSize.width, imageSize.height, 8, 4*imageSize.width, rgbColorSpace, kCGImageAlphaPremultipliedFirst);
    NSParameterAssert(context);
